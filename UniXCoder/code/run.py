...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            model.train()
            loss,logits,_ = model(inputs_ids_1,position_idx_1,attn_mask_1,inputs_ids_2,position_idx_2,attn_mask_2,labels)
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    for batch in tqdm(eval_dataloader):
        ...
        labels)=[x.to(args.device)  for x in batch]
        with torch.no_grad():
            lm_loss,logit,_ = model(inputs_ids_1,position_idx_1,attn_mask_1,inputs_ids_2,position_idx_2,attn_mask_2,labels)
...
def test(args, model, tokenizer, best_threshold=0):
    ...
    # Save confusion matrix as NumPy file (avoiding external dependencies)
    output_dir = os.path.dirname(args.output_dir) if hasattr(args, 'output_dir') else './outputs'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    # Try to save as visual plot if matplotlib and seaborn are available
    try:
        ...
        logger.info("Matplotlib or seaborn not available. Skipping visual plot.")
        logger.info("To create visual plots, install matplotlib and seaborn:")
        logger.info("    conda install -c conda-forge matplotlib seaborn")
        logger.info("    or")
        logger.info("    pip install matplotlib seaborn")
