            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if results['eval_f1']>best_f1:
                        ...
                        #np.save(os.path.join(args.output_dir,'outputs_and_embeddings.npy'),outputs_and_embeddings, allow_pickle=True)
...
def main():
    ...
    # Set seed
    set_seed(args)
    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_name_or_path.split('/')[-1]]
    ...
    if args.do_eval:
        ...
        model.to(args.device)
        results, embeddings = evaluate(args, model, tokenizer)
    if args.do_test:
        checkpoint_prefix = 'checkpoint-best-f1/model.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
        model.load_state_dict(torch.load(output_dir))
